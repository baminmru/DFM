...
class DataTreeBranchResourceIT {
    ...
    void checkStereoTypeIsRequired() throws Exception {
        int databaseSizeBeforeTest = dataTreeBranchRepository.findAll().collectList().block().size();
        // set the field null
        dataTreeBranch.setStereoType(null);

        // Create the DataTreeBranch, which fails.
        ...
        List<DataTreeBranch> dataTreeBranchList = dataTreeBranchRepository.findAll().collectList().block();
        assertThat(dataTreeBranchList).hasSize(databaseSizeBeforeTest);
    }
    ...
    void checkCaptionIsRequired() throws Exception {
        int databaseSizeBeforeTest = dataTreeBranchRepository.findAll().collectList().block().size();
        // set the field null
        dataTreeBranch.setCaption(null);
