...
import requestTypeResolve from './request-type-routing-resolve.service';

describe('RequestType routing resolve service', () => {
  let mockRouter: Router;
  let mockActivatedRouteSnapshot: ActivatedRouteSnapshot;
  let service: RequestTypeService;
  let resultRequestType: IRequestType | null | undefined;
  ...
  describe('resolve', () => {
    it('should return IRequestType returned by find', () => {
      ...
      TestBed.runInInjectionContext(() => {
        requestTypeResolve(mockActivatedRouteSnapshot).subscribe({
          next(result) {
    ...
    it('should return null if id is not provided', () => {
      ...
      expect(resultRequestType).toEqual(null);
    });

    it('should route to 404 page if data not found in server', () => {
      // GIVEN
      jest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse<IRequestType>({ body: null })));
      mockActivatedRouteSnapshot.params = { id: 123 };
