...
/**
 * REST controller for managing {@link com.bami.dfm.v1.domain.DataTreeBranch}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class DataTreeBranchResource {
    ...
    public ResponseEntity<DataTreeBranch> createDataTreeBranch(@RequestBody DataTreeBranch dataTreeBranch) throws URISyntaxException {
        ...
        DataTreeBranch result = dataTreeBranchRepository.save(dataTreeBranch);
    ...
    public ResponseEntity<DataTreeBranch> updateDataTreeBranch(
        ...
        if (!dataTreeBranchRepository.existsById(id)) {
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, dataTreeBranch.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<DataTreeBranch> partialUpdateDataTreeBranch(
        @PathVariable(value = "id", required = false) final Long id,
        @RequestBody DataTreeBranch dataTreeBranch
    ) throws URISyntaxException {
        ...
        Optional<DataTreeBranch> result = dataTreeBranchRepository
            ...
            .map(dataTreeBranchRepository::save);

        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, dataTreeBranch.getId().toString())
        );
    }
     ...
     */
    @GetMapping("/data-tree-branches")
    public List<DataTreeBranch> getAllDataTreeBranches(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get all DataTreeBranches");
        if (eagerload) {
            return dataTreeBranchRepository.findAllWithEagerRelationships();
        } else {
    ...
    public ResponseEntity<DataTreeBranch> getDataTreeBranch(@PathVariable Long id) {
        log.debug("REST request to get DataTreeBranch : {}", id);
        Optional<DataTreeBranch> dataTreeBranch = dataTreeBranchRepository.findOneWithEagerRelationships(id);
    ...
    public ResponseEntity<Void> deleteDataTreeBranch(@PathVariable Long id) {
        log.debug("REST request to delete DataTreeBranch : {}", id);
        dataTreeBranchRepository.deleteById(id);
        return ResponseEntity
            .noContent()
