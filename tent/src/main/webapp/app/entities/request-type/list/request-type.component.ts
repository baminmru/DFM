...
import { EntityArrayResponseType, RequestTypeService } from '../service/request-type.service';
...
export class RequestTypeComponent implements OnInit {
  ...
  trackId = (_index: number, item: IRequestType): number => this.requestTypeService.getRequestTypeIdentifier(item);
  ...
  ngOnInit(): void {
    this.subscription = combineLatest([this.activatedRoute.queryParamMap, this.activatedRoute.data])
      .pipe(
        ...
        tap(() => {
          if (!this.requestTypes || this.requestTypes.length === 0) {
            this.load();
          }
        }),
      )
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.requestTypes = this.refineData(dataFromBody);
  }

  protected refineData(data: IRequestType[]): IRequestType[] {
    const { predicate, order } = this.sortState();
    return predicate && order ? data.sort(this.sortService.startSort({ predicate, order })) : data;
  }

  protected fillComponentAttributesFromResponseBody(data: IRequestType[] | null): IRequestType[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.requestTypeService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
